...
def detect_communities(hybrid_graph: nx.Graph, tfidf_matrix=None) -> tuple[dict[int, list[str]], dict[str, int]]:
    ...
    # Fall back to KMeans only when (a) link structure gave us nothing usable, or
    # (b) it collapsed everything into one community on a corpus big enough to
    # warrant finer splitting -- and only if a TF-IDF matrix is available.
    needs_kmeans = (not communities_sets) or (len(communities_sets) == 1 and n >= 8)
    if needs_kmeans and tfidf_matrix is not None:
        ...
        if not communities_sets:
            # No link structure and no TF-IDF to fall back on: treat the whole
            # corpus as a single community rather than crashing.
            communities = {0: sorted(nodes)}
        else:
...
def build_community_centroids(communities: dict[int, list[str]], tfidf_matrix, note_index: dict[str, int]):
    ...
    for cid, members in communities.items():
        ...
        # sparse .mean(axis=0) returns the deprecated np.matrix; coerce to a plain
        # 2-D ndarray (1, V) so downstream cosine_similarity stays well-defined.
        centroids[cid] = np.asarray(sub.mean(axis=0))
    return centroids
