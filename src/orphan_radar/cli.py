...
def eval(
    ...
    config: Path | None = typer.Option(None, '--config', help='Optional JSON settings file.'),
    holdout: float = typer.Option(0.10, '--holdout', help='Fraction of existing links to hide (0-1).'),
    calibrate: bool = typer.Option(False, '--calibrate/--no-calibrate', help='Run random-search weight calibration on the reconstruction proxy.'),
    trials: int = typer.Option(40, '--trials', help='Number of weight vectors to try when calibrating.'),
):
    settings = RadarSettings.from_json_file(config)
    typer.echo(json.dumps(run_eval(src, out, settings, holdout_ratio=holdout, calibrate=calibrate, trials=trials), indent=2))
