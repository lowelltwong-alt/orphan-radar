...
from orphan_radar.eval.link_reconstruction import make_eval_summary
from orphan_radar.graph.build_graph import add_pagerank, build_link_graph
from orphan_radar.graph.communities import build_community_centroids, build_hybrid_graph, build_tfidf, detect_communities
...
def run_eval(source_dir: Path, output_dir: Path, settings: RadarSettings | None = None) -> dict:
    settings = settings or RadarSettings()
    notes, source_files = load_notes(source_dir.resolve(), settings)
    graph = build_link_graph(notes)
    result = make_eval_summary(graph)
